...
import com.wusy.studyroad.view.FullyGridLayoutManager;
import com.wusy.studyroad.view.FullyLinearLayoutManager;
...
public class ModuleView extends LinearLayout {
     ...
     * @param context
     * @param list 内容数据
     * @param listen Item的点击事件
     * @return ModuleView
     */
    public ModuleView showRecycelerView(Context context, List<ModuleViewBean> list, ModuleViewAdapter.OnItemClickListen listen){
        adapter=new ModuleViewAdapter(context,list);
        if(list.get(0).getLayoutNum()==adapter.LINEARLAYOUT){
        ...
        if(listen!=null) adapter.setOnItemClickListen(listen);
